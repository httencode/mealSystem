...
// 去除js和html的console.log const removeConsolelog =
// require("gulp-remove-logging");

/**
    ====================开发相关=========================
**/
gulp.task('test-clean_static', function () {
    gulp.src(['test/static/*'], {read: false})
        .pipe(plumber({
...
gulp.task('test-clean_devtool', function () {
    gulp.src(['test/devtool/*'], {read: false})
        .pipe(plumber({
...
gulp.task('test-script', function () {
    return gulp
        ...
        .pipe(babel())
        .pipe(webpack({
            // https://www.npmjs.com/package/webpack-stream entry: {     app: 'src/app.js',
            // test: 'test/test.js', }, output: {     filename: '[name].js', }, entry:
            // './src/js/main.js',
            output: {
                // filename: 'main.js'
            },
...
gulp.task('browser-sync', [
    ...
    browserSync.init({
        ...
        // open: true, host: '192.168.199.1',
        startPath: '/test/login.html', // 打开的位置
    });
    // 监听
    gulp.watch('src/css/**/*.scss', ['test-sass']);
    gulp.watch('src/js/**/*.js', ['test-script']);
    gulp.watch('src/*.html', ['test-copy-html']);
...
gulp.task('sass', function () {
    return gulp
        ...
        // .pipe(concat('main.css')) .pipe(assetRev())
        .pipe(cssnano())
...
gulp.task('script', function () {
    return gulp
        ...
        // .pipe(     removeConsolelog({         // Options (optional)         // eg: //
        // namespace: ['console', 'window.console']     }) )
        .pipe(babel())
        .pipe(webpack({
            // https://www.npmjs.com/package/webpack-stream entry: {     app: 'src/app.js',
            // test: 'test/test.js', }, output: {     filename: '[name].js', },
...
// 编译好的dest/js下的js文件压缩、压缩文件加后缀 gulp.task('js-compress', function () {
// gulp.src('dest/js/main.js')         .pipe(uglify())         .pipe(rename({
// suffix: '.min',         }))         .pipe(gulp.dest('dest/js')); });
// 编译好的dest/css下的css文件压缩、压缩文件加后缀 gulp.task('css-compress', function () {
// gulp.src('dest/css/main.css')         .pipe(cssnano())         .pipe(rename({
//             suffix: '.min',         })) .pipe(gulp.dest('dest/css')); });
// 生成最终上线代码
gulp.task('release', [
    'sass', 'script', 'copy-static', 'copy-lib', 'minihtml'
], function () {
    return notify({message: '大吉大利！代码release已完成！'});
});
